...
function testSuccess(type, actionCreator, expectedMessage) {
  test(`${type}, success`, () => {
    store.dispatch(actionCreator({ resource: 'test', params: { type: 'success' } })).then(
      mustCall((result) => {
...
function testSuccessWithBody(type, actionCreator, body, expectedMessage) {
  test(`${type}, success`, () => {
    store.dispatch(actionCreator({ resource: 'test', params: { type: 'success' }, body })).then(
      mustCall((result) => {
        assert.deepEqual(result.data, { message: expectedMessage });
      }),
      assert.fail
...
function testFailure(type, actionCreator, expectedErrorMessage) {
  test(`${type}, failure`, () => {
    store.dispatch(actionCreator({ resource: 'test', params: { type: 'fail' } })).then(
      assert.fail,
      mustCall((error) => {
